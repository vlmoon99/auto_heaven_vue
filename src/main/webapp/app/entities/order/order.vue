<template>
  <div>
    <h2 id="page-heading" data-cy="OrderHeading">
      <span v-text="$t('autoheavenApp.order.home.title')" id="order-heading">Orders</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('autoheavenApp.order.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'OrderCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('autoheavenApp.order.home.createLabel')"> Create a new Order </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && orders && orders.length === 0">
      <span v-text="$t('autoheavenApp.order.home.notFound')">No orders found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="orders">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('autoheavenApp.order.name')">Name</span></th>
            <th scope="row"><span v-text="$t('autoheavenApp.order.userId')">User Id</span></th>
            <th scope="row"><span v-text="$t('autoheavenApp.order.dateAdded')">Date Added</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="autoheavenApp.order.delete.question" data-cy="orderDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-order-heading" v-text="$t('autoheavenApp.order.delete.question', { id: removeId })">
