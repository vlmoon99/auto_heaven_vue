<template>
  <div>
    <h2 id="page-heading" data-cy="CategoryHeading">
      <span v-text="$t('autoheavenApp.category.home.title')" id="category-heading">Categories</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('autoheavenApp.category.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'CategoryCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('autoheavenApp.category.home.createLabel')"> Create a new Category </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && categories && categories.length === 0">
      <span v-text="$t('autoheavenApp.category.home.notFound')">No categories found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="categories">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('autoheavenApp.category.xmlCategoryId')">Xml Category Id</span></th>
            <th scope="row"><span v-text="$t('autoheavenApp.category.parentId')">Parent Id</span></th>
            <th scope="row"><span v-text="$t('autoheavenApp.category.name')">Name</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="autoheavenApp.category.delete.question" data-cy="categoryDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-category-heading" v-text="$t('autoheavenApp.category.delete.question', { id: removeId })">
